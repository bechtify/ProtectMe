...
const editContact = async (contactInfo, contactId) => {
  await contactStorage.updateContact(contactInfo, contactId);
};

// Service for DELETE /contacts/:userId
const deleteContacts = async (contactIds, userId) => {
  //return contactStorage.deleteContacts(contactInfo, contactId);
};
